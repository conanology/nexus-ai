...
const SKIP_INTEGRATION = !process.env.NEXUS_PROJECT_ID || !process.env.NEXUS_BUCKET_NAME;
describe.skipIf(SKIP_INTEGRATION)('GCP Infrastructure Integration', () => {
    ...
    describe('Firestore Operations', () => {
        it('should write and read a document', async () => {
            ...
            // Read
            const result = await firestore.getDocument(TEST_COLLECTION, TEST_DOC_ID);
        ...
        it('should return null for non-existent document', async () => {
            const result = await firestore.getDocument(TEST_COLLECTION, `nonexistent-${Date.now()}`);
        ...
        it('should query documents', async () => {
            ...
            try {
                // Query
                const results = await firestore.queryDocuments(TEST_COLLECTION, [{ field: 'status', operator: '==', value: 'test-query' }]);
    ...
    describe('Cloud Storage Operations', () => {
        it('should upload and download text content', async () => {
            ...
            // Upload
            const url = await storage.uploadFile(TEST_STORAGE_PATH, content, 'text/plain');
